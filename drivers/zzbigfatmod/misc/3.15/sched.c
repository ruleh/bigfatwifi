 ...
 * achieved as well.
 */
extern void kick_process(struct task_struct *p)
{
	int cpu;
 ...
 */
extern void preempt_notifier_register(struct preempt_notifier *notifier)
{
 ...
 */
extern void preempt_notifier_unregister(struct preempt_notifier *notifier)
{
...
extern void __kprobes add_preempt_count(int val)
{
...
extern void __kprobes sub_preempt_count(int val)
{
#ifdef CONFIG_DEBUG_PREEMPT
	/*
	 * Underflow?
 ...
 */
extern asmlinkage void __sched notrace preempt_schedule(void)
{
...
extern int default_wake_function(wait_queue_t *curr, unsigned mode, int wake_flags,
			  void *key)
{
 ...
 * changing the task state if and only if any tasks are woken up.
 */
extern void __wake_up_sync_key(wait_queue_head_t *q, unsigned int mode,
			int nr_exclusive, void *key)
{
	unsigned long flags;
 ...
 */
extern void __wake_up_sync(wait_queue_head_t *q, unsigned int mode, int nr_exclusive)
{
	__wake_up_sync_key(q, mode, nr_exclusive, NULL);
}
/*EXPORT_SYMBOL_GPL(__wake_up_sync);	 For internal use only <--then don't export it*/
 ...
 */
extern void __sched wait_for_completion(struct completion *x)
{
...
extern wait_for_completion_timeout(struct completion *x, unsigned long timeout)
{
 ...
 * interruptible.
 */
extern int __sched wait_for_completion_interruptible(struct completion *x)
{
 ...
 */
extern int __sched wait_for_completion_killable(struct completion *x)
{
...
extern void __sched sleep_on(wait_queue_head_t *q)
{
	sleep_on_common(q, TASK_UNINTERRUPTIBLE, MAX_SCHEDULE_TIMEOUT);
}

extern long __sched sleep_on_timeout(wait_queue_head_t *q, long timeout)
{
...
extern int __sched _cond_resched(void)
{
	if (should_resched()) {
		__cond_resched();
		return 1;
...
extern int __sched __cond_resched_softirq(void)
{
	BUG_ON(!in_softirq());
 ...
 */
extern bool __sched yield_to(struct task_struct *p, bool preempt)
{
 ...
 */
extern void __sched io_schedule(void)
{
	struct rq *rq = raw_rq();
